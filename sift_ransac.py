...
def sift_ransac(source, target, params):
    ...
    if echo:
        print(f"SIFT: Resampled source size: {resampled_source.size()}")
        print(f"SIFT: Resampled target size: {resampled_target.size()}")
    ...
    if echo:
        print(f"SIFT: Number of source keypoints: {len(source_keypoints)}")
        print(f"SIFT: Number of target keypoints: {len(target_keypoints)}")
    try:
    ...
    if echo:
        print(f"SIFT: Number of source points: {len(source_points)}")
        print(f"SIFT: Number of target points: {len(target_points)}")
    ...
    final_transform = utc.affine2theta(final_transform, (resampled_source.size(2), resampled_source.size(3))).type_as(source).unsqueeze(0)
    if echo:
        print(f"SIFT: Calculacted transform: {final_transform}")
    return final_transform
